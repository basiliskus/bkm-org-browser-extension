  ...
  if receivedMessage:
    ...
    if command == 'add-bookmark':
      ...
      if 'categories' in receivedMessage:
        ...
        bookmark.tags = [utils.get_tag_from_category(c) for c in receivedMessage['categories'] if c not in ignore_categories]
    ...
    if command == 'get-catalog':
      catalog = []
      bcc = BookmarkCollectionCatalog('default', collection_home)
